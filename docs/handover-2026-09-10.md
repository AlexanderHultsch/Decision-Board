...
stays with the thread, the answer may ask for pages once and a second pass
reads them, the picks screen in folded groups, the steps with real
numbers, the core skipping the vehicle concepts and the awarded volumes)
and 12 (one design system for every screen).
...
- The gate before every commit: `sh tools/precommit.sh` (the example config
  parses, the whole suite green: 305 tests at the 5.4 commit).
...
serves the groups), `POST .../read` (phase `asking`; when the answer
names `missing` pages, phase `reading_more` and one more call). The
snapshot's `steps` list is what the page shows while it runs.
...
12. Spec 5.4 was built against the fake model and the demo vault only.
    Alex's real run of the two questions that started it ("which VPDS
    tasks is [name] responsible for", then "the exact wording of the tasks
    you listed") is the acceptance test.
