...
archive. Spec sections 3 to 11 are built; nothing in the specification is
outstanding.
...
- The gate before every commit: `sh tools/precommit.sh` (the example config
  parses, the whole suite green: 282 tests at commit `439808d`).
- The browser walk (Playwright, Chromium preinstalled in the cloud
  ...
  server sees the first run's work. Stop it with `pkill -f demo_server.py`.
- Commit messages, PR bodies, code and comments carry no model name.

## Layout (at commit `439808d`)
...
src/programmind/
  ...
  knowledge/                knowledge.py (selection, the roles-folder lookup), picker.py, enrich.py, evaluate.py, prompts/
...
Ask the vault thread, told apart by `kind`. A topic keeps the paths of the
vault pages it was sent, never their text. `server.history_folder`
overrides the folder; the older `server.threads_folder` is still read.
