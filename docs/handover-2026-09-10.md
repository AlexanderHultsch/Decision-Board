...
volumes), 5.5 (quality first: no budget for Ask the vault, whole pages, a
checker call after every answer and a loop that reads what it names, up
to `ask.max_reads` rounds within `ask.max_read_tokens` per read) and 12
...
- The gate before every commit: `sh tools/precommit.sh` (the example config
  parses, the whole suite green: 308 tests at the 5.5 commit).
...
An Ask the vault question runs: `POST .../question` (the choosing call,
phase `choosing`), the `picks` phase (Alex reads or cancels; `estimate`
serves the groups), `POST .../read`, then the loop of spec 5.5: phase
`asking` (whole pages within the ceiling, one answer call), `checking`
(the checker call, `picker.check`), and when it names pages `reading_more`
and `asking` again, until nothing is wanted or `ask.max_reads` is reached.
The turn records every round (`rounds`), what the ceiling left out
(`left`) and what the check still wanted at the cap (`still_wanted`).
The snapshot's `steps` list is what the page shows while it runs.
...
12. Spec 5.5 was built against the fake model and the demo vault only.
    ...
    those tasks") is the acceptance test. Watch the checker: if it names
    nothing when pages are plainly missing, the prompt
    `knowledge_check.md` is where to work, not Python.
