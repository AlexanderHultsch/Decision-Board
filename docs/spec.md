...
| **KPIs live inside a member's own responsibilities**, never in a member of their own (decided 9 September 2026: the "KPI Check" member was removed). The instruction to align with them is in the conduct note, once, not in any role description | Each swim lane is held to its own measures and is the one that knows them. A separate scorekeeper reads the same options with none of the responsibility, which is a weaker answer than the swim lane's own. |
...
| **A member may be defined by more than one file**: files naming the same member are merged, and identity (title, icon, colour, order) stays with the file carrying the role headings, or the one not marked `kind: addendum` — never with whichever name sorts first. Whether a member is "filled" is decided after merging | An official role description, owned by the organisation, must stay byte-for-byte as written. What the board needs and the official document does not carry goes in a companion note beside it (`_Template - member addendum.md`). |
